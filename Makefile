...
NVCC_FLAGS+=--default-stream per-thread -m64 -O3 --use_fast_math --extra-device-vectorization -std=c++17  -Xptxas --warn-on-local-memory-usage,--warn-on-spills, --generate-line-info 
...
$(TARGETS): %: %.cu
	$(NVCC) -o $@.app $< $(NVCC_FLAGS) -I$(CUFFTDX_INCLUDE_DIR)
