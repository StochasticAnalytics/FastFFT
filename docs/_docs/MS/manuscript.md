...
The Fast Fourier transform (FFT) is one of the most widely used and heavily optimized digital signal processing algorithms. Many of the image processing algorithms, like those used in cryo-Electron Microscopy, rely on the FFT to accelerate convolution operations central to image alignment and also in reconstruction algorithms that operate most accurately in Fourier space. FFT libraries like FFTW and cuFFT provide routines for highly-optimized general purpose multi-dimensional FFTs; however, this generality comes at the cost of optimality for several use-cases where only a subset of the input or output points are required. We demonstrate that algorithms based on the transform decomposition approach are well suited to the memory hierarchy of modern GPUs by implementing them in CUDA/C++ using the cufftdx header-only library. The results have practical implications, accelerating several key image processing algorithms by factors of 3-10x over those built using Nvidia’s general purpose FFT library cuFFT. These include movie-frame alignment, image resampling via Fourier cropping, 2d and 3d template matching, and subtomogram averaging and alignment.

## Introduction

The Discrete Fourier Transform (DFT) and linear filtering, *e.g.* convolution, are among the most common operations in digital signal processing. It is therefore assumed that the reader has a basic familiarity with Fourier Analysis how it is applied in their respective fields; we will focus here on digital image processing for convenience. For a detailed introduction to mathematics underlying the DFT, the reader is referred to the free book by Smith {cite:p}`smith_mathematics_2008`. 

There are many ways to implement the computation of the DFT, the most simple of which is a matrix multiplication, which scales in computational complexity as $O(n^2)$. The Fast Fourier Transform (FFT) employs a recursive approach, as described in the following section, that in turn reduces the computational complexity to $O(nlog(n))$. This reduction in number of arithmetic operations leads to both an increase in the computational speed, as well as reduced roundoff error. 

Many authors, particularly of earlier papers, use the exact reduction in arithmetic operations as a measure of the optimality for their particular FFT algorithm. A mathematical lower bound on the number of operations [⚠️] is found in the Winograd algorithm ⚠️, however, the fastest algorithms are ... ( something about power of three as well ⚠️). Modern FFT algorithms depend strongly on data locality, and therfore the goal of popular FFT library's is to provide optimal routines for FFTs of any size that are adaptable to the idiosyncrasies of the memory hierarchy for the particular computer hardware they are being run on ⚠️. 

(⚠️ TODO: Intro figure 1). For certain applications that have very specific computational demands, particularly academic or industrial research that require high performance computing resources, this generality providing for every possible FFT is not strictly needed. Substantial financial and environmental resources could be conserved by using specialized hardware or software. 

- Mention GPUs TPUs FPGAs other co-processors (cite benchmark paper)
- Ballpark of time savings and energy savings
